...
std::string readFileToStr(const std::string &filename) {
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg.starts_with("--target=")) {
      ...
      std::cerr << "Usage: " << argv[0] << " <filename> [--target=PLATFORM]\n";
      std::cerr
          << "Available targets: windows, linux, macos (default: native)\n";
      return 1;
    }
  }
  ...
  try {
    ...
    auto [success, resolved_ast] = analyzer.resolveAST();
    if (!success) {
      std::println(
          "\033[31;1;4merror:\033[0m exiting due to previous error(s)");
      return 1;
    ...
    if (target_platform == "windows") {
      ...
      std::println("clang --target=x86_64-pc-windows-msvc {} -o output.exe",
                   ir_filename);
    } else if (target_platform == "linux") {
      std::println("Linux target IR generated. You can cross-compile with:");
      std::println("clang --target=x86_64-unknown-linux-gnu {} -o output",
                   ir_filename);
    } else if (target_platform == "macos") {
      std::println("macOS target IR generated. You can cross-compile with:");
      std::println("clang --target=x86_64-apple-darwin {} -o output",
                   ir_filename);
    } else {

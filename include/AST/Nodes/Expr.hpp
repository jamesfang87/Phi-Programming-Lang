...
public:
  ...
  void setType(const TypeRef &T) { Type = T; }
...
public:
  ...
  [[nodiscard]] auto *getAsStruct() {
    assert(llvm::isa<StructDecl>(Decl));
    return llvm::dyn_cast<StructDecl>(Decl);
  }
  ...
  void setActiveVariant(VariantDecl *Variant) {
    ...
    assert(llvm::isa<EnumDecl>(getDecl()) && "Decl must be enum");
...
public:
  ...
  [[nodiscard]] const FieldDecl *getField() const { return Field; }

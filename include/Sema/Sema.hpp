...
public:
   ...
   */
  std::pair<bool, std::vector<std::unique_ptr<FunDecl>>> resolveAST();

  // EXPRESSION VISITORS
  /// Resolves integer literal expressions (trivial, sets type to appropriate
  /// integer type)
...
private:
   ...
   * @return true if resolution succeeded, false otherwise
   */
  static bool resolveFunDecl(FunDecl *fun);
   ...
   */
  static bool resolveTy(std::optional<Type> type);
   ...
   * @return true if resolution succeeded, false otherwise
   */
  static bool resolveParamDecl(ParamDecl *param);

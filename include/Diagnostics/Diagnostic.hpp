...
struct DiagnosticStyle {
   ...
   */
  explicit DiagnosticStyle(const Color c, const bool bold = false,
...
struct DiagnosticLabel {
   ...
   */
  DiagnosticLabel(SrcSpan span, std::string message,
                  const DiagnosticStyle style = DiagnosticStyle(),
                  const bool is_primary = false)
      : span(std::move(span)), message(std::move(message)), style(style),
...
public:
  ...
  Diagnostic &with_primary_label(const SrcSpan &span, std::string message) {
    labels.emplace_back(span, std::move(message), get_style_for_level(level),
                        true);
    return *this;
  }
   ...
   * @return Diagnostic& Reference for method chaining
   */
  Diagnostic &with_secondary_label(const SrcSpan &span, std::string message,
   ...
   * @return Diagnostic& Reference for method chaining
   */
  Diagnostic &with_suggestion(const SrcSpan &span, std::string replacement,
                              std::string description) {
    suggestions.emplace_back(span, std::move(replacement),
  ...
  [[nodiscard]] const std::vector<DiagnosticLabel> &get_labels() const {
    return labels;
  }
  ...
  [[nodiscard]] const std::vector<DiagnosticSuggestion> &
  get_suggestions() const {
    return suggestions;
  }
  ...
  [[nodiscard]] bool has_primary_labels() const {
    return std::ranges::any_of(
        labels, [](const DiagnosticLabel &label) { return label.is_primary; });
  }

...
bool TypeChecker::visit(BinaryOp &E) {
  ...
  case TokenKind::DoubleAmp: {
    if (!Lhs.isPrimitive() || Lhs.asPrimitive() != BuiltinKind::Bool) {
    ...
    if (!Rhs.isPrimitive() || Rhs.asPrimitive() != BuiltinKind::Bool) {
      error(std::format("Operation `{}` can only be applied to bool type",
                        TokenKindToStr(E.getOp())))
...
bool TypeChecker::visit(UnaryOp &E) {
  ...
  case TokenKind::Bang: {
    ...
    if (!Type.isPrimitive() ||
        E.getOperand().getType().asPrimitive() != BuiltinKind::Bool) {

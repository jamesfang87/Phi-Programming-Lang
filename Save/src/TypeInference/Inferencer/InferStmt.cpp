...
TypeInferencer::InferRes TypeInferencer::visit(Block &B) {
  ...
  return {AllSubsts, Monotype::makeCon(BuiltinKind::Null)};
}
...
TypeInferencer::InferRes TypeInferencer::visit(ReturnStmt &S) {
  const auto ExpectedType = CurFunRetType.empty()
                                ? Monotype::makeCon(BuiltinKind::Null)
                                : CurFunRetType.back();
  if (S.hasExpr()) {
    auto [Subst, ActualType] = visit(S.getExpr());
    unifyInto(Subst, ActualType, ExpectedType);
    recordSubst(Subst);
    return {Subst, Monotype::makeCon(BuiltinKind::Null)};
  }

  auto Subst = Substitution();
  unifyInto(Subst, Monotype::makeCon(BuiltinKind::Null), ExpectedType);
  return {Subst, Monotype::makeCon(BuiltinKind::Null)};
}
...
TypeInferencer::InferRes TypeInferencer::visit(DeclStmt &S) {
  visit(S.getDecl());
  return {Substitution{}, Monotype::makeCon(BuiltinKind::Null)};
}
